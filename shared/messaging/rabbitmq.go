		...
		for msg := range msgs {
			if err := tracing.TracedConsumer(msg, func(ctx context.Context, delivery amqp.Delivery) error {
				...
				if err != nil {
					...
					headers["x-orgin-exchange"] = delivery.Exchange
					headers["x-orgin-routing-key"] = delivery.RoutingKey
...
func (r *RabbitMQ) PublishMessage(ctx context.Context, routeKey string, message contracts.AmqpMessage) error {
	log.Printf("Publishing message with route key: %v", routeKey)
	...
	return tracing.TracedPublisher(ctx, TripExchange, routeKey, msg, r.publish)
}

func (r *RabbitMQ) publish(ctx context.Context, exchange, routeKey string, msg amqp.Publishing) error {
