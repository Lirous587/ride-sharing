...
func main() {
	...
	// Stripe processer
	paymentProcesser := stripe.NewStripeClient(stripeCfg)

	service := service.NewPaymentService(paymentProcesser)
	...
	// Trip Consumer
	tripConsumer := events.NewTripConsumer(rabbitmq, service)
